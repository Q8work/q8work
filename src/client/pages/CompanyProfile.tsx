        ...
        <div className="space-y-6 lg:col-span-2">
          ...
          <div>
            <h2 className="mb-4 text-lg font-extrabold text-brand-darkest">
              فرص العمل المتاحة {user && jobs.length > 0 && <span className="text-brand">({jobs.length})</span>}
            </h2>
            {!user ? (
              <div className="card flex flex-col items-center gap-3 py-10 text-center">
                ...
                <div>
                  <p className="font-bold text-brand-darkest">فرص هذه الشركة متاحة للأعضاء</p>
                  <p className="mt-1 text-sm text-brand">سجّل مجاناً لعرض الفرص والتقديم عليها.</p>
                </div>
                <Link to={`/register?redirect=${encodeURIComponent(`/companies/${id}`)}`} className="btn-primary mt-1">سجّل لعرض الفرص</Link>
              </div>
            ) : jobs.length === 0 ? (
